...
public enum MysqlColumnType {
    ...
    MysqlColumnType(String name, String javaType, String jsType) {
        this.name = name;
    ...
    public static Object formatValue(String type, String value) throws Exception {
        ...
        for (MysqlColumnType sqlTypeEnum : values()) {
    ...
    public static String toJavaType(String type) throws Exception {
        if(StrUtil.isEmpty(type)){
            return MysqlColumnType.CHAR.javaType;
        }
        String dbType = type.toUpperCase();
        int i = dbType.indexOf("(");
        if (i > 0) {
            dbType = dbType.substring(0, i);
        }
        for (MysqlColumnType sqlTypeEnum : values()) {
            if (sqlTypeEnum.name.equals(dbType)) {
